...
CREATE PROCEDURE [dbo].[dbdocs_CleanUpSchema]
(
...
# 2021-07-01 - MChrupczalski - Added Error Handling and @OutMsg NVARCHAR(MAX) output parameter
    ...
    BEGIN TRY
        ...
        SELECT @SQL =   
            ...
            WHERE SO.schema_id = schema_id('''+ @SchemaName +''')
                AND SO.type IN (''PK'', ''FN'', ''TF'', ''TR'', ''V'', ''U'', ''P'',''IF'',''SO'')
            ORDER BY 
                CASE WHEN type = ''PK'' THEN 1 
        ...
        SET @OutMsg = 'All objects and schema [' + @SchemaName + '] were removed from the database.';
        RETURN 1;
    END TRY

    BEGIN CATCH
        SET @OutMsg = 'Error occurred, during removal of objects and schema [' + @SchemaName + ']; ErrMsg: ' + ERROR_MESSAGE();
        RETURN 0
    END CATCH
