...
-----
Last Modified: 27/06/2021 04:nn:42
   WeekDay: Sunday
...
WITH CTE_Data AS(
    ...
    FROM sys.all_objects AS ao
        ...
        LEFT JOIN (SELECT major_id, [value] FROM sys.extended_properties WHERE (name = 'MS_Description' AND class_desc = 'OBJECT_OR_COLUMN')) AS x
    ...
    FROM sys.schemas AS s
        LEFT JOIN (SELECT major_id, [value] FROM sys.extended_properties WHERE (name = 'MS_Description' AND class_desc = 'SCHEMA')) AS x
    ...
    FROM sys.table_types AS t
        ...
        LEFT JOIN (SELECT major_id, [value] FROM sys.extended_properties WHERE (name = 'MS_Description' AND class_desc = 'TYPE')) AS x
    ...
    FROM sys.database_principals AS p
        LEFT JOIN (SELECT major_id, [value] FROM sys.extended_properties WHERE (name = 'MS_Description' AND class_desc = 'DATABASE_PRINCIPAL')) AS x
...
SELECT * INTO [dbo].[dbdocs_MajorObjectsInfo] 
FROM CTE_Data